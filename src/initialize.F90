...
!===============================================================================
  ...
  subroutine initialize_run()
    ...
    if (run_mode /= MODE_PLOTTING) then
      ...
      ! Allocate sensitivity tally
      call allocate_sensitivities()
...
!===============================================================================
  ...
  subroutine initialize_mpi()
    ...
    integer                   :: bank_blocks(12)   ! Count for each datatype
#ifdef MPIF08
    type(MPI_Datatype)        :: bank_types(12)
    type(MPI_Datatype)        :: result_types(1)
    type(MPI_Datatype)        :: temp_type
#else
    integer                   :: bank_types(12)    ! Datatypes
    integer                   :: result_types(1)  ! Datatypes
    integer                   :: temp_type        ! temporary derived type
#endif
    integer(MPI_ADDRESS_KIND) :: bank_disp(12)     ! Displacements
    ...
    call MPI_GET_ADDRESS(b % ifp_id,         bank_disp(6), mpi_err)
    call MPI_GET_ADDRESS(b % nuclide_born,   bank_disp(7), mpi_err)
    call MPI_GET_ADDRESS(b % energy_fission, bank_disp(8), mpi_err)
    call MPI_GET_ADDRESS(b % energy_born,    bank_disp(9), mpi_err)
    call MPI_GET_ADDRESS(b % mtnum_born,     bank_disp(10), mpi_err)
    call MPI_GET_ADDRESS(b % mesh_born,      bank_disp(11), mpi_err)
    call MPI_GET_ADDRESS(b % mesh_born_fm,   bank_disp(12), mpi_err)
...
!===============================================================================
! ALLOCATE_SENSITIVITIES allocates memory for sensitivities
!===============================================================================

  subroutine allocate_sensitivities()
    ...
    READ_SENSITIVITIES :do i = 1, n_sens
      ...
      allocate(t % results(3, t % n_nuclide_bins, &
      ...
      if (t % method == 1) then
         ...
         allocate(t % neutronfission(3 * n_particles))
         t % results        = ZERO
         t % neutrontally   = ZERO
         t % secondtally    = ZERO
         t % cumtally       = ZERO
        ...
        t % poleNeutrontally = ZERO
      end if

     ! if method == 2, i.e. CLUTCH method using imp generated by IFP
      if (t % method == 2) then
         ! used to calculate importance distribution
         allocate(t % neutrontally(1, 1, &
                  t % imp_mesh_bins, 1, 3 * n_particles))
         allocate(t % secondtally(t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins, MAX_SECONDARY))
         allocate(t % cumtally(t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
         allocate(t % clutchsen(t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
         allocate(t % neutronvalue(3 * n_particles))
         allocate(t % neutronfission(t % imp_mesh_bins)) !T*V/F=IMP
         allocate(t % importance(t % imp_mesh_bins))
         ...
         ! Multipole sensitivity data structures
         allocate(t % poleNeutrontally(MAX_PARAMS, MAX_POLES, 1, &
                  1, t % imp_mesh_bins, 1,  3 * n_particles))
         allocate(t % poleSecondtally(MAX_PARAMS,MAX_POLES,t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins,  MAX_SECONDARY))
         allocate(t % poleCumtally(MAX_PARAMS,MAX_POLES,t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
         allocate(t % poleClutchsen(MAX_PARAMS,MAX_POLES,t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
        t % poleResults      = ZERO
        t % poleCumtally     = ZERO
        t % poleSecondtally  = ZERO
        t % poleNeutrontally = ZERO
        t % poleClutchsen    = ZERO

      end if

      ! if method == 3, i.e. CLUTCH method using imp generated by fission matrix
      if (t % method == 3) then
         ! used to calculate importance distribution
         allocate(t % secondtally(t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins, MAX_SECONDARY))
        ...
        t % poleClutchsen    = ZERO
         ...
         if (i == 1) then     ! allocate fission matrix in the first sensitivity
            allocate(fismatrix)
            fismatrix_on = .true.
            allocate(fismatrix % fm_mesh)
            fismatrix % fm_mesh = m
            fismatrix % fm_dimension = t % imp_mesh_bins
            allocate(fismatrix % greenterm(fismatrix % fm_dimension, &
                                           fismatrix % fm_dimension))
            allocate(fismatrix % source(fismatrix % fm_dimension))
            allocate(fismatrix % eigenvector(fismatrix % fm_dimension))
            fismatrix % greenterm = ZERO
            fismatrix % source = ZERO
            fismatrix % eigenvector = ZERO
          end if

      end if

      ! if method == 4 i.e. IFP method for GPT, allocate more memory * 3
      if (t % method == 4) then
      ...
      ! if method == 5, i.e. gpt-CLUTCH method using imp generated by gpt-IFP
      if (t % method == 5) then
         ! used to calculate importance distribution
         allocate(t % neutrontally(3 * n_particles, 1, 1, &
                  t % imp_mesh_bins, 1))
         allocate(t % secondtally(MAX_SECONDARY, &
                  t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
         ...
         allocate(t % clutchsen(t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
         allocate(t % gptvaluenumer(3 * n_particles))
         allocate(t % gptvaluedenom(3 * n_particles))
         allocate(t % neutronfission(t % imp_mesh_bins)) !T*V/F=IMP
         allocate(t % importance(t % imp_mesh_bins))
         ...
         t % cumtally       = ZERO
         t % clutchsen      = ZERO
         t % gptvaluenumer  = ZERO
         t % gptvaluedenom  = ZERO
         t % neutronfission = ZERO ! tally*value/fission=importance
         t % importance     = ZERO

      end if

      ! if method == 6, i.e. gpt-CLUTCH method using imp generated by fission matrix
      if (t % method == 6) then
         ! used to calculate importance distribution
         allocate(t % secondtally(MAX_SECONDARY, &
                  t % n_nuclide_bins, t % n_score_bins, &
         ...
         allocate(t % clutchsen(t % n_nuclide_bins, t % n_score_bins, &
                  t % n_mesh_bins, t % n_energy_bins))
         allocate(t % tallynumer(t % imp_mesh_bins)) ! using mesh_born filter
         allocate(t % tallydenom(t % imp_mesh_bins)) ! using mesh_born filter
         allocate(t % importance(t % imp_mesh_bins))
      ...
      NUCLIDE_LOOP: do j = 1, t % n_nuclide_bins
        nuclides(t % nuclide_bins(j)) % calculate_derivative = .true.
      end do NUCLIDE_LOOP


     end do READ_SENSITIVITIES

     progenitornum = 3 * work_index(rank)  ! the initial ifp_id of process rank


     RESPONSETALLY_LOOP: do i = 1, n_resptallies
      FILTER_LOOP: do j = 1, size(resp_tallies(i) % filters)
        call resp_tallies(i) % filters(j) % obj % initialize()
      end do FILTER_LOOP
    end do RESPONSETALLY_LOOP


  end subroutine allocate_sensitivities
